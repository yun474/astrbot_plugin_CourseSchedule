...
一个为 AstrBot 设计的课程表插件：用 WakeUp 分享口令或 `.ics` 课表文件绑定后，可以查询自己和群友的课程安排、本周上课排行，并支持 AI 主动调用；在 QQ 官方机器人（官bot）上还可以用 Markdown 卡片 + 按钮的形式展示。

## ✨ 功能特性

*   **两种绑定方式**：发送 `/绑定课表` 后，发一条 **WakeUp 课程表分享口令** 或上传 `.ics` 日历文件即可，支持群聊和私聊，可自定义显示昵称。
*   **绑定码复用**：绑定成功后会得到一个绑定码，在其他群或私聊发送 `/关联课表 绑定码` 即可直接复用，不用重复上传（官bot 群聊无法直接发文件时尤其有用）。
*   **今日 / 明日课表**：查看自己今天还有什么课、明天有什么课，以精美图片展示；带上 `@某人` 还能看同群群友的课表。
...
| `/绑定课表 [昵称]` | 发送后 60 秒内在当前会话发送 WakeUp 分享口令或上传 `.ics` 文件完成绑定。昵称可选，官bot 拿不到昵称时建议填写。 |
| `/关联课表 [绑定码] [昵称]` | 用绑定码把已绑定的课表复用到当前会话；不带参数时显示自己的绑定码。 |
| `/解绑课表` | 解除当前会话的课表绑定。 |
| `/查看课表 [@某人]` | 查看今天还有什么课。带 `@某人`、openid/QQ 号或群友昵称可查看同群其他人的课表。 |
| `/查看明日课表 [@某人]` | 查看明天有什么课，同样支持查看他人。 |
...
插件注册了一个函数工具 `course_schedule_query(scope, user_id)`：

*   `scope` 取 `today` / `tomorrow` / `week`。
*   `user_id` 可选，填 openid / QQ 号或群友昵称即可查询同群其他人的课表；不填则查询提问的人。

工具只把对应范围内的全部课程（含起止时间、地点）和当前日期时间返回给模型，由模型自行判断哪些课已结束、正在进行或尚未开始（工具本身不直接给用户发消息），因此可以直接问：
...
未绑定课表时，工具会返回绑定引导。查询他人只在对方已在当前群绑定课表时有效，跨群和私聊互相隔离。
...
> 官bot 群聊里无法直接给机器人发文件，可以直接发 WakeUp 口令，或先在私聊绑定，再到群里发送 `/关联课表 绑定码`。

## ❓ 如何获取 WakeUp 口令或 .ics 文件？

*   **WakeUp 口令**：在 **WakeUp 课程表** 中选择“分享课表”→“分享给好友”，把生成的整段分享文案直接发给机器人即可（口令 30 分钟内有效）。
*   **.ics 文件**：在课表软件的设置中选择“导出”，导出为日历文件（`.ics`）。
...
*   `wakeup_client.py` — WakeUp 6.x 分享口令协议（来自上游 [Futureppo 的实现](https://github.com/advent259141/astrbot_plugin_CourseSchedule/pull/8)）及分享数据到 ICS 的转换。
